...
void Vehicle::update(double deltaTime){
    // // if (m_vehicle) std::cout << "Vehicle pos " << m_vehicle->GetChassis()->GetPos() << "\n";
    // // m_vehicle->EnableRealtime(true);
    // if (true){
    //     std::cout << "Vehicle pos " << m_vehicleHandle->vehicle->GetChassis()->GetPos() << "\n";
    //     // // std::cout << "Steearing angle: " << m_vehicleHandle->vehicle->GetSteeringAngle() << "\n";

    //     // auto engine = m_vehicleHandle->vehicle->GetEngine();
    //     // if (engine) {
    //     //     std::cout << "Vehicle engine: " << engine->GetOutputMotorshaftTorque() << "\n";
    //     // } else {
...
void Vehicle::onDrawGizmos(rendering::GizmosManager *gizmos){
    ...
    gizmos->draw_cube(physics::toV3d(vehicle->GetPos()), .25, glm::vec4(1, 0, 0, 1));

    // Draw chasis
    gizmos->draw_cube(physics::toV3d(vehicle->GetChassisBody()->GetPos()), .51, glm::vec4(1, 0, 0, 1));
...
void Vehicle::loadVehicleModelJSON(){
    ...
    assert(!m_vehicleModelPath.empty() && "Tried to load Vehicle JSON but vehicleModelPath not defined");
    // assert(!m_vehicle.vehicle && "Tried to load Vehicle JSON has already been loaded");  // Can only load once
    assert(m_rigidBody && "Tried to load Vehicle JSON but rigidbody not defined");  // Can only load once

    // Load model

    Physics* physics = m_scene->getPhysics();

    // m_vehicle = chrono_types::make_unique<chrono::vehicle::WheeledVehicle>(&physics->m_system, m_vehicleModelPath, true, true);
    ...
    m_rigidBody->hardResetBody((chrono::ChBody*)vehicle->GetChassisBody().get());
}
