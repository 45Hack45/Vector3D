...
Physics::Physics() {
    ...
    m_terrain = chrono_types::make_shared<chrono::vehicle::FlatTerrain>(-2, .8f);

    // double terrainLength = 50.0;
    // auto rigid_terrain = chrono_types::make_shared<chrono::vehicle::RigidTerrain>(&m_system);
    // // auto patch_mat = chrono_types::make_shared<chrono::ChContactMaterialSMC>();
    // // patch_mat->SetFriction(0.9f);
    // // patch_mat->SetRestitution(0.01f);
    // // patch_mat->SetYoungModulus(2e7f);
    // // patch_mat->SetPoissonRatio(0.3f);
    // // auto patch = rigid_terrain->AddPatch(patch_mat, chrono::ChCoordsys<>(chrono::ChVector3d(0, -5, 0), chrono::QUNIT), terrainLength, terrainLength);
...
// chrono::vehicle::WheeledVehicle* Physics::createVehicle(std::string vehicleModelPath){
//     // Create vehicle and driverInput
//     auto& vehicle = m_vehicles.emplace_back(&m_system, vehicleModelPath, true, true);
//     auto& driverInputs = m_driverInputs.emplace_back();
//     return &vehicle;
...
VehicleHandle Physics::createVehicle(std::string vehicleModelPath){
    ...
    // double braking
    // double clutch
    m_vehicleInputs.push_back({ VehicleRaw_ptr(m_vehicles, m_vehicles.size() - 1), chrono::vehicle::DriverInputs({0, 1, 0, 0}) });

    return VehicleHandle(m_vehicleInputs, m_vehicleInputs.size() - 1);  // Provide pointer to Vehicle 
}
...
void Physics::stepSimulation(){
    ...
    //     // TODO: Store WheeledVehicle component instead and get DriverInputs from there
    //     vehicle.Synchronize(time, vehicle, *m_terrain);
    // }
    ...
    // for (auto vehicle : m_vehicles){
    //     vehicle.Synchronize(time, chrono::vehicle::DriverInputs{steering, throttle, braking, clutch}, *m_terrain);;
    // }

    // // // // for (auto vehicleInput : m_vehicleInputs){
    // // // //     // vehicleInput.vehicle->Synchronize(time, vehicleInput.driverInputs, *m_terrain);
    // // // //     vehicleInput.vehicle->Synchronize(time, chrono::vehicle::DriverInputs{steering, throttle, braking, clutch}, *m_terrain);
    // // // // }
